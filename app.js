...
// 라우터 미들웨어
    ...
    router.get('/postsDetail', isLoggedIn, (req,res)=>{  
        ...
        if(!req.query.id){
            ...
            conn.query('SELECT id,title,img,content,nick,updatedAt FROM posts WHERE id=?'
...
router.get('/postsList', isLoggedIn, (req,res)=>{
    ...
    router.post('/addPosts', (req,res)=>{
        ...
        conn.query('INSERT INTO posts(title,content,nick,updatedAt) VALUES(?,?,?,now())'
                ,[title, content, nick], (err, result)=>{
            if(err){
                console.log(err);
                res.end();
            }else{
...
app.listen(80, function () {
    ...
    router.get('/updatePosts', isLoggedIn, (req,res)=>{
        ...
        const id = parseInt(req.query.id);
        conn.query('SELECT id,title,content FROM posts WHERE id=?'
                ,[id],(err,rs)=>{
            if(err){
                console.log(err);
                res.end();
        ...
        conn.query('SELECT nick FROM posts WHERE id=?'
            ,[id],(err,rs)=>{
                if(err){
                    ...
                    const nick = rs[0].nick;
                    if(nick == req.user.nick){
                        conn.query('UPDATE posts SET title=?,content=? WHERE id=?'
                                ,[title, content, id],(err,rs)=>{
                            if(err){
                                console.log(err);
                                res.end();
                            }else{
    ...
    router.get('/scrapPosts', isLoggedIn, (req,res)=>{
        ...
        const id = parseInt(req.query.id);
        console.log(id);
        conn.query('SELECT id,title,content,img,hit,nick FROM posts WHERE id=?'
                ,[id],(err,rs)=>{
            if(err){
                console.log(err);
                res.end();
    ...
    router.post('/scrapPosts',(req,res)=>{
       ...
       // const free_pw = req.body.free_pw;
        ...
        conn.query('INSERT INTO scraps(postId, userId, title, nick, hit) VALUES(?,?,?,?,?)'
                ,[postId,userId,title,nick,hit],(err,rs)=>{
                    if(err){
                        console.log(err);
                        res.end();
                    }else{
    ...
    router.get('/scrapsList', isLoggedIn, (req,res)=>{
        ...
        conn.query('SELECT COUNT(*) AS cnt FROM scraps where userId=?',[userId],(err,result)=>{  //전체 글목록 행 갯수 구하기
            ...
            conn.query('SELECT id, postId, title, nick, hit FROM scraps where userId= ? ORDER BY updatedAt ASC LIMIT ?,?'
